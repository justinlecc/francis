...
class MessageReceiver():
	...
	def sms(self, from_phone_number, text):
		...
		from_human = self.db.session.query(Humans).filter_by(phone_number=from_phone_number).first()
		
		# Create the human if it does not exist
		if from_human is None:
			from_human = Humans(from_phone_number)
			self.db.session.add(from_human)
			self.db.session.commit()

		# Insert the sms into the db
		message = Messages(text, from_human.id) # TODO: make 'text' db safe?
